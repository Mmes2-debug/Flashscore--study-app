...
const CommunityVoting: React.FC<CommunityVotingProps> = ({ currentUser }) => {
  ...
  useEffect(() => {
    loadVotingTopics();
    if (currentUser) {
      const balance = piCoinManagerInstance.getBalance(currentUser.id);
  ...
  const castVote = (topicId: string, optionId: string, coinsToSpend: number) => {
    ...
    piCoinManagerInstance.spendCoins(
      currentUser.id,
      coinsToSpend,
    ...
    // Update user balance
    const newBalance = piCoinManagerInstance.getBalance(currentUser.id);
